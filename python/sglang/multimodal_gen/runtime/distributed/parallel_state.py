...
_DECODE: GroupCoordinator | None = None
    ...
    **kwargs,
    ...
    if parallel_mode == "pipeline":
        ...
        return GroupCoordinator(
            ...
            local_rank=local_rank,
            torch_distributed_backend=backend,
            group_name="decode_group" if parallel_mode == "decode" else "cfg_group",
        )
...
def initialize_model_parallel(
    ...
    global _DECODE
    assert _DECODE is None, "decode parallel group is already initialized"
    _DECODE = init_parallel_group_coordinator(
        group_ranks=rank_generator.get_ranks("tp-sp-pp-cfg"),
        local_rank=get_world_group().local_rank,
        backend=backend,
        parallel_mode="decode",
    )
...
def get_decode_parallel_group_coordinator() -> GroupCoordinator:
    assert _DECODE is not None, "decode parallel group is not initialized"
    return _DECODE
...
def destroy_model_parallel() -> None:
    ...
    global _TP, _SP, _DP, _CFG, _PP, _DECODE, _DIT, _VAE

    for group in (_TP, _SP, _DP, _CFG, _PP, _DECODE):
    ...
    _TP, _SP, _DP, _CFG, _PP, _DECODE, _DIT, _VAE = (None,) * 8
